...
import com.example.btd.domain.use_cases.DeleteFileFromAbsenceUseCase

class DeleteFileFromAbsenceConverter {
    fun convert(result: Result<DeleteFileFromAbsenceUseCase.Response>): UiState<String> =
        when (result) {
            is Result.Error -> {
                UiState.Error(
                    result.exception
